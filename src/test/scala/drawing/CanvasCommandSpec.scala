...
class CanvasCommandSpec extends Specification {
  "A DrawLine command" should {
    "accept only horizontal or vertical lines" in {
      DrawLineCommand.build(Coordinates(1,1), Coordinates(1,2)) match {
        case \/-(command) => {
      ...
      DrawLineCommand.build(Coordinates(1,2), Coordinates(3,2)) match {
        case \/-(command) => {
      ...
      DrawLineCommand.build(Coordinates(1,1), Coordinates(3,2)) match {
